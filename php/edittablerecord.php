			...
			while($row = $result->fetch_assoc())
	  		{
	  			$hid = $row["Hotel_Id"];
	  			$tno = $row["Table_No"];
	  			$tprice = $row["Table_Price"];
	  		}
		} 
		else echo "<script>alert('No items selected')</script>";
	?>

	<form method="post" action = "submitedittablerecord.php?id=<?php echo $id; ?>" class="frm">
		Hotel Name:</br>
		<input type="text" name="hid" value="<?php echo $hid; ?>" required><br/><br/>
		Table Number:</br>
		<input type="text" name="tableno" value="<?php echo $tno; ?>" required><br/><br/>
		Table Price per person:</br>
		<input type="text" name="tableprice" value="<?php echo $tprice; ?>" required><br/><br/>
